...
Коротко: Telegram-проект или бот по теме «tg ai finance».

## Что здесь

- Назначение: Telegram-проект или бот по теме «tg ai finance».
...
- `storage.py` - работа с PostgreSQL базой данных
...
Для деплоя на Railway необходимо настроить PostgreSQL:
...
3. Теперь при каждом редеплое данные не будут стираться, так как PostgreSQL предоставляет постоянное хранилище

**Важно:** Без PostgreSQL база данных будет стираться при каждом редеплое из-за эфемерной файловой системы Railway
